...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry
{
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected)
    {
        ...
        String s3 = flag2 ? EnumChatFormatting.DARK_RED + this.field_148301_e.gameVersion : this.field_148301_e.populationInfo;
        int j = this.mc.fontRendererObj.getStringWidth(s3);
        this.mc.fontRendererObj.drawString(s3, x + listWidth - j - 15 - 2, y + 1, 8421504);
        int k = 0;
        String s = null;

        if (!this.field_148301_e.serverCommand.isEmpty())
        {
            String s2 = Color.c(5) + "/" + this.field_148301_e.serverCommand + " ";
            LabyMod.getInstance().draw.drawString(s2, (double)(315 + x), (double)(y + 1));
        }
        ...
        if (l1 >= listWidth - 15 && l1 <= listWidth - 5 && i1 >= 0 && i1 <= 8)
        {
            this.field_148303_c.setHoveringText(s1);
        }
        else if (l1 >= listWidth - j - 15 - 2 && l1 <= listWidth - 15 - 2 && i1 >= 0 && i1 <= 8)
        {
